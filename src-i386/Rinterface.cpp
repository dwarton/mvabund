...
// [[Rcpp::export]]
List RtoAnovaCpp(const List &rparam, RcppGSL::Matrix &Y, RcppGSL::Matrix &X,
  ...
  List rs = List::create(
      Named("multstat") =
          NumericVector(anova.multstat, anova.multstat + nModels - 1),
      Named("Pmultstat") =
          NumericVector(anova.Pmultstat, anova.Pmultstat + nModels - 1),
...
List RtoGlmAnova(const List &sparam, const List &rparam, RcppGSL::Matrix &Y,
  ...
  unsigned long int dif =
      floor((double)(clk_end - clk_start) / (double)(CLOCKS_PER_SEC));
...
// declare the function to be 'exported' to R
// [[Rcpp::export]]
List RtoGlm(const List &rparam, RcppGSL::Matrix &Y, RcppGSL::Matrix &X,
            RcppGSL::Matrix &O) {
  ...
  List rs = List::create(
      ...
      Named("theta") =
          NumericVector(glmPtr[mtype]->theta, glmPtr[mtype]->theta + Y.ncol()),
      Named("two.loglike") =
          NumericVector(glmPtr[mtype]->ll, glmPtr[mtype]->ll + Y.ncol()),
      Named("aic") =
          NumericVector(glmPtr[mtype]->aic, glmPtr[mtype]->aic + Y.ncol()),
      Named("deviance") =
          NumericVector(glmPtr[mtype]->dev, glmPtr[mtype]->dev + Y.ncol()),
      Named("iter") = NumericVector(glmPtr[mtype]->iterconv,
                                    glmPtr[mtype]->iterconv + Y.ncol()));
...
List RtoGlmSmry(const List &sparam,                   // model params list
  ...
  RcppGSL::MatrixView usig =
      gsl_matrix_submatrix(myTest.smryStat, 1, 1, nParam, nVars);
  RcppGSL::MatrixView uPsig =
      gsl_matrix_submatrix(myTest.Psmry, 1, 1, nParam, nVars);

  List rs = List::create(Named("multstat") = gsl_vector_get(&mult.vector, 0),
                         Named("Pmultstat") = gsl_vector_get(&Pmul.vector, 0),
                         Named("unitmult") = umult, Named("Punitmult") = uPmul,
                         Named("signific") = signi, Named("Psignific") = Psign,
                         Named("unitsign") = usig, Named("Punitsign") = uPsig,
                         Named("nSamp") = myTest.nSamp,
                         Named("aic") =
                             NumericVector(myTest.aic, myTest.aic + nVars));
...
// [[Rcpp::export]]
List RtoSmryCpp(const List &rparam, RcppGSL::Matrix &Y, RcppGSL::Matrix &X,
                Rcpp::Nullable<RcppGSL::Matrix> &bID) {
  // pass regression parameters
  mv_Method mm;
  mm.nboot = as<unsigned int>(rparam["nboot"]);
  ...
  RcppGSL::MatrixView usigni =
      gsl_matrix_submatrix(smry.unitstat, 1, 0, nParam, nVars);
  RcppGSL::MatrixView uPsign =
      gsl_matrix_submatrix(smry.Punitstat, 1, 0, nParam, nVars);

  List rs = List::create(
      Named("multstat") = smry.multstat[0],
      Named("Pmultstat") = smry.Pmultstat[0],
      Named("signific") =
          NumericVector(smry.multstat + 1, smry.multstat + nParam + 1),
      Named("Psignific") =
          NumericVector(smry.Pmultstat + 1, smry.Pmultstat + nParam + 1),
      Named("unitmult") = umulti, Named("Punitmult") = uPmult,
      Named("unitsign") = usigni, Named("Punitsign") = uPsign,
      Named("nSamp") = smry.nSamp, Named("R2") = smry.R2);
