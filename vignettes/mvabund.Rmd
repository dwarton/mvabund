...
If you do not have a specific hypothesis in mind that you want to test, and are instead interested in which model terms are statistically significant, then the `summary()` function will come in handy. 

```{r}
summary(cope.nb) 
```

If obtaining predicted values from the model is the goal, you can use the `predict()` function. Note that `type = response` will produce values on the scale of the response variable (i.e. counts)
